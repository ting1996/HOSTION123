...
function ExecQuery(data, deviceID = "doquangtan") {
  return new Promise ((resolve, reject) => {
      if (connection == null || connection.state == "disconnected") {
        ConnecttoDatabase("doquangtan", "1", function(x){
          if (x == 1) {
            ...
            reject("Mysql error: khong the truy van mysql!");
          }
        });
      } else {
          ExecuteQuery(data, function(err, rows){
...
io.on("connection", function(socket){ 
  ...
  socket.on('all-notification-update', function (data) {
    io.sockets.emit('all-notification-update', data);
  } );
...
app.post('/api/devices/getToken', (req, res) => {
  console.log(req.query);
  return res.status(200).send('OK');
})
...
let jsonPaser = bodyParser.json(options);

app.post('/api/devices/chamcong', jsonPaser, (req, res) => {
  let data = req.body;
  switch (data.dataType) {
    case 1: {
      console.log(data);
      return res.status(200).send('fingger');
    } break;
    case 2: {
      try {
        if (data.uid == null
        ...
        || data.imageUrl == null) {
          ...
          fb = fb.substr(0, fb.length - 2);
          return res.status(400).send({error: 'data is missing ' + fb});
        }
        
        let query = 'SELECT * FROM USERSAUTHENTICATIONDATA WHERE \'!\?!\' LIKE CONCAT(`UID`, \'%\'); ';
        query = query.replace('!\?!', data.uid);
        ExecQuery(query, data.deviceID)
        .then((d) => {
          if (d.length > 0) {
            //Upload image
            let nameSaveToServer = randomstring.generate(15) + ".jpg";
            (async () => {
              let imageName = data.imageUrl;
              let response = await fetch('http://houston123.edu.vn:5000/static/image/' + imageName + '.jpg')
              while (1) {
                imageName = randomstring.generate(15) + ".jpg";
                if (!fs.existsSync(__dirname + '/public/img/hinhupload/' + imageName)) {
                  break;
                }
              }
              let path = __dirname + '/public/img/hinhupload/' + imageName;
              ...
              fileStream.on('finish', () => {
                (async () => {
                  while (1) {
                    try {
                      await webDavclient.stat("/Public/img/timekeeper" + nameSaveToServer)
                      .then(() => {
                        nameSaveToServer = randomstring.generate(15) + ".jpg";
                      })
                    } catch (e) {
                      break;
                    }
                  ...
                  let imageData = fs.readFileSync(__dirname + "/public/img/hinhupload/" + imageName);
                  await webDavclient.putFileContents("/Public/img/timekeeper/" + nameSaveToServer, imageData, {format: "binary", overwrite: true})
                  .then((v) => {
                    try {
                      fs.removeSync(__dirname + "/public/img/hinhupload/" + imageName);
                    } catch (e) {
                    }
            ...
            .then (() => {
              //Upload data to timekeeper
              query = 'INSERT INTO `TIMEKEEPER` (`code`, `time`, `inout`, `image`) VALUES (\'!\?!\', \'!\?!\', \'!\?!\', \'!\?!\'); '
              query = query.replace('!\?!', d[0].code);
              query = query.replace('!\?!', data.time.y + '-' + ("0" + data.time.mo).slice(-2) + '-' + ("0" + data.time.d).slice(-2) + ' ' + ("0" + data.time.h).slice(-2) + ':' + ("0" + data.time.m).slice(-2) + ':' + ("0" + data.time.s).slice(-2));
              query = query.replace('!\?!', '1');          
              query = query.replace('!\?!', nameSaveToServer);
              ExecQuery(query, data.deviceID)
              .then(() => {
            ...
            return Promise.reject(new Error('Thẻ không tồn tại trong hệ thống!'));
          }
        })
        .catch((err) => {
    ...
    default:
      return res.status(400).send({error: 'the dataType "' + req.body.dataType + '" is not supported!'});
  }
})
  ...
  try {
    ...
    form.on('fileBegin', function (name, file){
      while (1) {
        ...
        if (type == null && file.type == 'image/jpeg') {
          type = 'jpg'
        }
        new_name = randomstring.generate(15) + "." + type;
